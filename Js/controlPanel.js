class ControlPanel {
  constructor(beatwriter, playParameterValues, beatTrackParameterValues) {
    ...
    this.autoBpm = 0;
    this.autoBpmDisplay = document.getElementById("auto-bpm-display");
 this.autoBpmX2Button = document.getElementById("auto-bpm-x2-button");
    this.autoBpmHalfButton = document.getElementById("auto-bpm-half-button");
    this.autoBpmCommitButton = document.getElementById("auto-bpm-commit-button");
    ...
    this.playValueSelector = new ValueSelector(this, beatwriter.gridView, document.getElementById('lcd-container'), beatwriter.playParameterValues, this.beatwriter.fileManager);
    this.beatTrackValueSelector = new ValueSelector(this, beatwriter.gridView, document.getElementById('beat-track-container'), beatwriter.beatTrackParameterValues, this.beatwriter.fileManager);
this.calculator = new BpmCalculator();
...
this.autoBpmX2Button.addEventListener('click', () => this.handleAutoBpmX2ButtonClicked());
    this.autoBpmHalfButton.addEventListener('click', () => this.handleAutoBpmHalfButtonClicked());
    this.autoBpmCommitButton.addEventListener('click', () => this.handleAutoBpmCommitButtonClicked());
  ...
  handleAutoBpmX2ButtonClicked() {
    const currentBpm = parseFloat(this.autoBpmDisplay.textContent);
    const doubledBpm = currentBpm * 2;
    this.autoBpm = doubledBpm.toFixed(2);
    this.autoBpmDisplay.textContent = this.autoBpm;
  }

  handleAutoBpmHalfButtonClicked() {
    const currentBpm = parseFloat(this.autoBpmDisplay.textContent);
    const halvedBpm = currentBpm / 2;
    this.autoBpm = halvedBpm.toFixed(2);
    this.autoBpmDisplay.textContent = this.autoBpm;
    
  }

  handleAutoBpmCommitButtonClicked() {
    this.beatwriter.bpm.currentValue = parseFloat(this.autoBpmDisplay.textContent);
    this.updateDisplays();
  }
